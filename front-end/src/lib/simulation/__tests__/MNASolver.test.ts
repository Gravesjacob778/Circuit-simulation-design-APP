...
describe('MNASolver', () => {
  ...
  describe('Extended Component Models', () => {
    ...
    it('should analyze circuit with LED (simplified model)', () => {
      // 電路: DC 5V -> LED (V_f=2.0V) -> R1 100Ω -> GND
      // LED 使用二極體模型：V_f = 2.0V, R_on = 0.1Ω
      // 電流 = (5V - 2.0V) / (100Ω + 0.1Ω) ≈ 29.97mA
      ...
      expect(r1Current).toBeDefined();
      expect(ledCurrent).toBeCloseTo(0.02997, 3); // ~29.97mA (new diode model)
